...
cv::Mat HomographyEstimator::normalizePoints(std::vector<cv::Point2f>& points) {
    // Compute centroid
    ...
    scale = std::sqrt(2.0f) * points.size() / scale;
    ...
    // Normalize points
    for (auto& pt : points) {
        float x = scale * (pt.x - centroid.x);
        float y = scale * (pt.y - centroid.y);
        pt = cv::Point2f(x, y);
    }
...
void HomographyEstimator::denormalizeHomography(cv::Mat& H, const cv::Mat& T1, const cv::Mat& T2) {
    H = T2.inv() * H * T1;
}